...
import { Shield, Truck, FileText, BarChart3, Settings, LogOut, Package } from "lucide-react";
      ...
      <aside className="w-64 flex-shrink-0 border-r border-slate-800 bg-surface/50 backdrop-blur-xl flex flex-col">
        ...
        <div className="p-4 flex-1">
          <nav className="space-y-1">
            ...
            <Link href="/logistica/proveedores" className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${pathname.includes('/proveedores') ? 'bg-rose-500/10 text-rose-400' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}>
              <Package className="h-4 w-4" /> Proveedores y Entregas
            </Link>
          </nav>
        </div>
