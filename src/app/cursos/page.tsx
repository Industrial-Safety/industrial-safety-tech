...
async function getCourses(): Promise<CourseFromAPI[]> {
  try {
    ...
    if (!res.ok) {
      console.error(`[/cursos] Backend respondió ${res.status} en GET /api/v1/course`);
      return [];
    }
    const data = await res.json();
    return Array.isArray(data) ? data : [];
  } catch (err) {
    console.error("[/cursos] No se pudo conectar al backend:", err);
    return [];
  }
}
          ...
          {courses.length === 0 ? (
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {courses.map((course) => {
                ...
                const price = course.details?.precio
                  ? `$${course.details.precio.toFixed(2)}`
                  : "$0.00";
