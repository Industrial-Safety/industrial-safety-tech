...
export default function CoursePlayerPage({ params }: { params: Promise<{ cursoId: string }> }) {
  ...
  const handleSubmitPost = async (e: React.FormEvent) => {
    ...
    try {
      const body = {
        authorId: (session as any).keycloakId ?? session.user?.email ?? "unknown",
        authorName: session.user?.name ?? "Trabajador",
  ...
  if (!course) {
    return (
      <div className="flex flex-col items-center justify-center h-screen text-muted">
        ...
        <Link href="/trabajador/learning"><Button variant="outline" className="mt-4">Volver</Button></Link>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] -m-6 lg:-m-8">
      ...
      <div className="h-14 bg-surface border-b border-border flex items-center px-4 justify-between shrink-0">
        <div className="flex items-center gap-4">
          <Link href="/trabajador/learning" className="flex items-center text-muted hover:text-foreground transition-colors">
                        ...
                        <div key={post.id}>
                          ...
                          {replyTo === post.id && (
                            <form onSubmit={handleSubmitPost} className="mt-3 ml-12 flex gap-3 items-center">
                              <Avatar src={session?.user?.image ?? undefined} fallback={(session?.user?.name ?? "T")[0]} />
